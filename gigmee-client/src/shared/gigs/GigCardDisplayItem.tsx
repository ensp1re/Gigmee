...
const GigCardDisplayItem: FC<IGigCardItems> = ({
  ...
  const gigTitle: string = replaceSpacesWithDash(gig?.title);
  ...
  const onSaveGigTitle = (gig: ISellerGig): void => {
  ...
  useEffect(() => {
    ...
    socket.on("online", (data: string[]) => {
      sellerUsername.current = find(data, (name: string) => {
        name === gig?.username;
      }) as string;
    });
  }, [gig?.username]);
  ...
  return (
    <div className="rounded">
      <div className="mb-8 flex cursor-pointer flex-col gap-2">
        <Link
          onClick={() => onSaveGigTitle(gig)}
          to={`/gig/${lowerCase(gig?.username as string)}/${gigTitle}/${gig?.sellerId}/${gig?.id}/view`}
        >
          <LazyLoadImage
            src={gig?.coverImage}
            alt="Gig cover image"
          ...
          <div className="flex w-full justify-between">
            <span className="text-md hover:underline">
              {linkTarget ? (
                <Link
                  onClick={() => onSaveGigTitle(gig)}
                  to={`$/seller_profile/${lowerCase(gig?.username as string)}/${gig?.sellerId}/${
