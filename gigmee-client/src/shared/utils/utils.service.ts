...
export const getDataFromLocalStorage = (key: string): void => {
...
export const countriesList = (): string[] => {
  const countriesObject: LocalizedCountryNames<{ select: 'official' }> =
...
export const orderTypes = (
  ...
  const orderList: IOrderDocument[] = filter(
    ...
    (order: IOrderDocument) => {
      return lowerCase(order.status) === lowerCase(status);
    },
  );
