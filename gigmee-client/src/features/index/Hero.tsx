...
import  {categories}  from "src/shared/utils/utils.service";
...
const Hero: FC = (): ReactElement => {
  ...
  const navigateToToSearchPage = (): void => {
    const url = `/gigs/search?${createSearchParams({query: searchTerm.trim()})}`;
    navigate(url);
  }

  const sequence = categories().reduce<(string | number)[]>(
    (acc, category, index) => {
      acc.push(category);
  ...
  return (
    <div className="relative bg-white pb-20 pt-40 dark:bg-gray-900 lg:pt-44">
      <div className="relative m-auto px-6 xl:container md:px-12 lg:px-6">
        ...
        <h1 className="text-center text-4xl font-black text-green-900 dark:text-white sm:mx-auto sm:w-10/12 sm:text-5xl md:w-10/12 md:text-5xl lg:w-auto lg:text-left xl:text-7xl">
          Hire expert freelancers <br className="hidden lg:block" />{" "}
          <span className="relative bg-gradient-to-r from-green-600 to-green-400 bg-clip-text text-transparent dark:from-green-400 dark:to-lime-300">
            for your project
          </span>
          .
        </h1>
          ...
          <div className="relative mt-8 space-y-8 text-center sm:mx-auto sm:w-10/12 md:mt-16 md:w-2/3 lg:ml-0 lg:mr-auto lg:w-7/12 lg:text-left">
            ...
            <div className="flex w-full justify-between gap-6 lg:gap-12">
              ...
              onSubmit={(e: FormEvent) => {
                e.preventDefault();
                navigateToToSearchPage();
              }}
              className="mx-auto flex w-full rounded-full  items-center bg-white">
                ...
                <div className="bg-green-500 rounded-r-full">
                  <Button
                    type="submit"
                    ...
                    label={<FaSearch 
                      className="h-5 w-5"
                    />}
                    onClick={navigateToToSearchPage}
                  />
                </div>
              </form>
            </div>
                  ...
                  >
                    <div className="flex justify-center">
                      <span className="block truncate font-medium dark:text-white">
                        <a href={`/search/categories/${replaceSpacesWithDash(category)}`}>
                          {category}
                        </a>
                      </span>
                    </div>
