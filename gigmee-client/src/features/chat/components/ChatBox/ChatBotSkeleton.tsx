...
const ChatBotSkeleton:FC = ():ReactElement => {
  return (
    <div
    ...
    <div className="h-[500px] overflow-y-scroll md:h-full">
        <div className="my-2 flex h-[280px] flex-col overflow-hidden px-4 md:h-[72%] animate-pulse">
            ...
            <div key={uuidv4()} className={`my-2 flex max-w-[300px] gap-y-6 text-sm ${msg % 2===0
                ? 'flex-row-reverse self-end' : '' }`}>
                <div className={`rounded-full bg-gray-200 h-10 w-10 ${msg % 2===0 ? 'hidden' : '' }`}></div>
                <p className={`ml-2 w-[200px] rounded-[10px] bg-gray-200 px-4 py-2 text-start text-sm font-normal
                    md:w-[220px] ${ msg % 2===0 ? 'w-[200px] rounded-[10px] bg-gray-200 text-white py-5' : '' }`}></p>
            </div>
            ))}
        </div>
    </div>
