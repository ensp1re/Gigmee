...
const BuyerTable: FC<IOrderTableProps> = ({
  ...
  const updatedOrders = filter(orders, (order) => lowerCase(order.status) === lowerCase(type));
          ...
          <>
            <thead className="border-grey border-b text-xs uppercase text-gray-700 sm:[&>*:not(:first-child)]:hidden">
              {updatedOrders.map(() => (
                <tr
                  key={uuidv4()}
                  className="mb-1 flex flex-col flex-nowrap bg-green-500 text-white sm:mb-0 sm:table-row md:table-row lg:bg-transparent lg:text-black"
                >
            ...
            <tbody className="flex-1 sm:flex-none">
              {updatedOrders.map((order: IOrderDocument) => (
                <tr
                  key={uuidv4()}
                  ...
                  <td className="p-3 text-left">
                    <div className="grid">
                      <Link
                        ...
                        className="truncate text-sm font-normal hover:text-green-500"
                      >
                        {order.gigBasicTitle}
                      </Link>
                    </div>
