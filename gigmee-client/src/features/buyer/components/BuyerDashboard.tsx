...
const BuyerDashboard: FC = (): ReactElement => {
  const [type, setType] = useState<string>(BUYER_GIG_STATUS.IN_PROGRESS);
        ...
        <div className="p-0">
          <ul className="flex w-full cursor-pointer list-none flex-col flex-wrap rounded-[2px] sm:flex-none sm:flex-row">
            <li
              ...
              onClick={() => setType(BUYER_GIG_STATUS.IN_PROGRESS)}
            >
              <a
                href="#activeorders"
                className={`px-4 py-3 text-xs text-[#555555] no-underline sm:text-sm md:text-base ${
                  type === BUYER_GIG_STATUS.IN_PROGRESS
              ...
              >
                ...
                {orderTypes(BUYER_GIG_STATUS.IN_PROGRESS, orders) > 0 && (
                  <span className="ml-1 rounded-[5px] bg-green-500 px-[5px] py-[1px] text-xs font-medium text-white">
                    {shortenLargeNumbers(
