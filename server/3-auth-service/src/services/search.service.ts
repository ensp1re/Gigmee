import { elasticSearchClient, getGigById } from '@auth/elasticsearch';
import { SearchResponse } from '@elastic/elasticsearch/lib/api/types';
import { IHitsTotal, IPaginateProps, IQueryList, ISearchResult, ISellerGig } from '@ensp1re/gigme-shared';

export async function gigById(index: string, gigId: string): Promise<ISellerGig> {
  const gig: ISellerGig = await getGigById(index, gigId);
  return gig;
}
