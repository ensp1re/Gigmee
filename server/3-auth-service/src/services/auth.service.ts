...
import { IAuthBuyerMessageDetails, IAuthDocument, firstLetterUppercase, lowerCase, winstonLogger } from '@ensp1re/gigme-shared';
...
export async function createAuthUser(data: IAuthDocument): Promise<IAuthDocument | undefined> {
  try {
    ...
    // delete password from data using library omit
...
export async function getAuthUserByVerificationToken(token: string): Promise<IAuthDocument | undefined> {
  try {
    ...
    }) as IAuthDocument | null;

    return user || undefined; // Return `undefined` if no user is found
  } catch (error) {
    log.error('Error fetching user by verification token:', error);
    console.log(error);
  }
}
